  ...
  constructor(_props) {
    ...
    _defineProperty(this, "initialize", props => {
      ...
      const addSpring = (anchor, tension, influence, buckets = permBuckets) => {
        buckets[0].push(withInfluence(influence, this.target, springBehavior(this.dt, this.target, this.obj, anchor, tension)));
      };

      const addFriction = (damping, influence, buckets = permBuckets) => {
        buckets[1].push(withInfluence(influence, this.target, frictionBehavior(this.dt, this.target, this.obj, damping)));
      };

      const addGravity = (anchor, strength, falloff, influence, buckets = permBuckets) => {
        buckets[0].push(withInfluence(influence, this.target, gravityBehavior(this.dt, this.target, this.obj, anchor, strength, falloff)));
      };
      ...
      if (props.dragWithSpring) {
        ...
        dragBuckets[0].push(anchorBehavior(this.dt, this.target, this.obj, dragAnchor));
      }
      ...
      if (props.boundaries) {
        snapBuckets[0].push(bounceBehavior(this.dt, this.target, this.obj, props.boundaries, props.boundaries.bounce));
      ...
      const stopWhenNeeded = cond(and(greaterOrEq(noMovementFrames.x, ANIMATOR_PAUSE_CONSECUTIVE_FRAMES), greaterOrEq(noMovementFrames.y, ANIMATOR_PAUSE_CONSECUTIVE_FRAMES)), [props.onStop ? cond(clockRunning(this.clock), call([this.target.x, this.target.y], ([x, y]) => props.onStop({
        nativeEvent: {
          x,
          y
        }
      }))) : undefined, stopClock(this.clock)], startClock(this.clock));
      ...
      const trans = (axis, vaxis, lowerBound, upperBound) => {
        ...
        let advance = cond(lessThan(abs(vx), ANIMATOR_PAUSE_ZERO_VELOCITY), x, add(x, multiply(vx, this.dt)));
        ...
        const step = cond(eq(this.state, State.ACTIVE), [cond(dragging, 0, [handleStartDrag, startClock(this.clock), set(dragging, 1), set(start, x)]), set(anchor, add(start, drag)), cond(this.dt, dragBehaviors[axis])], [cond(dragging, [updateSnapTo, set(dragging, 0)]), cond(this.dt, snapBehaviors[axis]), testMovementFrames, stopWhenNeeded]);
        const wrapStep = props.dragEnabled ? cond(props.dragEnabled, step, [set(dragging, 1), stopClock(this.clock)]) : step; // export some values to be available for imperative commands
