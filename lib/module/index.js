  ...
  constructor(_props) {
    ...
    _defineProperty(this, "initialize", props => {
      ...
      const trans = (axis, vaxis, lowerBound, upperBound) => {
        ...
        const step = cond(eq(this.state, State.ACTIVE), [cond(dragging, 0, [handleStartDrag, startClock(clock), set(dragging, 1), set(start, x)]), set(anchor, add(start, drag)), cond(dt, dragBehaviors[axis])], [cond(dragging, [updateSnapTo, set(dragging, 0)]), cond(dt, snapBehaviors[axis]), testMovementFrames, stopWhenNeeded]);
