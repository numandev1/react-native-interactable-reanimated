  ...
  constructor(_props) {
    ...
    _defineProperty(this, "initialize", props => {
      ...
      const trans = (axis, vaxis, lowerBound, upperBound) => {
        ...
        const step = cond(eq(this.state, _reactNativeGestureHandler.State.ACTIVE), [cond(dragging, 0, [handleStartDrag, startClock(this.clock), set(dragging, 1), set(start, x)]), set(anchor, add(start, drag)), cond(this.dt, dragBehaviors[axis])], [cond(dragging, [updateSnapTo, set(dragging, 0)]), cond(this.dt, snapBehaviors[axis]), testMovementFrames, stopWhenNeeded]);
