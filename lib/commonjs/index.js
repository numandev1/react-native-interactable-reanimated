...
class Interactable extends _react.Component {
  constructor(props) {
    ...
    const handleStartDrag = props.onDrag && call([target.x, target.y], ([x, y]) => props.onDrag({
      nativeEvent: {
        x,
        y,
        state: 'start'
    ...
    const updateSnapTo = snapTo(tossedTarget, props.snapPoints, snapAnchor, props.onSnap, props.onDrag);
    ...
    const stopWhenNeeded = cond(and(greaterOrEq(noMovementFrames.x, ANIMATOR_PAUSE_CONSECUTIVE_FRAMES), greaterOrEq(noMovementFrames.y, ANIMATOR_PAUSE_CONSECUTIVE_FRAMES)), [props.onStop ? cond(clockRunning(clock), call([target.x, target.y], ([x, y]) => props.onStop({
      nativeEvent: {
        x,
        y
      }
    ...
    const trans = (axis, vaxis, lowerBound, upperBound) => {
      ...
      const testMovementFrames = block([onChange(snapAnchor.x, set(last, Number.MAX_SAFE_INTEGER)), onChange(snapAnchor.y, set(last, Number.MAX_SAFE_INTEGER)), cond(eq(advance, last), set(noMoveFrameCount, add(noMoveFrameCount, 1)), [set(last, advance), set(noMoveFrameCount, 0)])]);
      const step = cond(eq(state, _reactNativeGestureHandler.State.ACTIVE), [cond(dragging, 0, [handleStartDrag, startClock(clock), set(dragging, 1), set(start, x)]), set(anchor, add(start, drag)), cond(dt, dragBehaviors[axis])], [cond(dragging, [updateSnapTo, set(dragging, 0)]), cond(dt, snapBehaviors[axis]), testMovementFrames, stopWhenNeeded]);
